...
<style>
    /* 
     * This stylesheet is designed to format raw HTML generated from Markdown.
     * All styles are scoped to the `.doc-content` class to prevent conflicts.
     */
    ...
    /* For inline code snippets, e.g., `my_function()` */
    .doc-content code {
        background-color: var(--bs-tertiary-bg);
        color: var(--bs-danger);
        /* Provides good contrast in both themes */
        padding: 0.2em 0.4em;
    ...
    .doc-content pre {
        ...
        /* Standard dark background for code blocks is best for syntax highlighting */
        padding: 1.25rem;
    ...
    /* Reset inline styles when `code` is inside a `pre` block */
    .doc-content pre code {
        background-color: transparent;
        color: #abb2bf;
        /* Light text color suitable for the dark background */
        padding: 0;
        font-size: inherit;
        /* Inherit the font size from the <pre> tag */
    }
    ...
    .doc-content tbody tr:nth-of-type(odd) {
        background-color: var(--bs-tertiary-bg);
    }

    html[data-bs-theme="dark"] .doc-content tbody tr:nth-of-type(odd) {
        background-color: var(--bs-body-bg);
        /* Use a subtler striping for dark mode */
    }
...
7.2. <code>php spark train</code><br />
7.3. <code>php spark [another:command]</code></p><br />
<p><strong>8. Configuration Reference</strong><br />
8.1. Application (<code>App.php</code>)<br />
8.2. Database (<code>Database.php</code>)<br />
8.3. Custom Configurations (<code>AGI.php</code>, etc.)</p><br />
...
<p>The GenAI Web Platform is a comprehensive, multi-functional application built on the CodeIgniter 4 framework. It serves as a portal for registered users to access a suite of powerful digital services, including AI-driven content generation and analysis, real-time cryptocurrency data queries, and robust user and content management capabilities. Designed with a modular architecture, it features a secure user authentication system, an account dashboard with an integrated balance and payment system (supporting M-Pesa, Airtel, and Card), and a complete administrative panel for user oversight.</p><br />
...
<li><strong>AI Service Integration:</strong> Advanced text and multimedia interaction with Google's Gemini API, featuring a sophisticated conversational memory system.</li><br />
...
<li><strong>Administrative Dashboard:</strong> Robust tools for user management, balance adjustments, financial oversight, and sending email campaigns to all users.</li><br />
...
<ul><br />
<li><strong>MVC-S Architecture:</strong> The application separates concerns into Models (database), Views (presentation), Controllers (request handling), and Services (business logic).</li><br />
...
<p>The project extends the traditional Model-View-Controller (MVC) pattern with a <strong>Service layer (MVC-S)</strong> to better organize business logic.</p><br />
<ul><br />
<li><strong>Models (<code>app/Models</code>)</strong>: Handle all direct database interactions. They are responsible for querying, inserting, and updating data.</li><br />
<li><strong>Views (<code>app/Views</code>)</strong>: Contain the presentation logic (HTML). They receive data from controllers and render it for the user.</li><br />
<li><strong>Controllers (<code>app/Controllers</code>)</strong>: Act as the bridge between Models and Views. They handle incoming HTTP requests, orchestrate calls to services, and pass data to the appropriate view.</li><br />
<li><strong>Services (<code>app/Libraries</code>)</strong>: Contain the core business logic. This includes interacting with third-party APIs (Paystack, Gemini), processing complex data, and performing calculations. This keeps controllers lean and focused on handling the request-response cycle.</li><br />
</ul><br />
<h4><strong>4.2. The Request Lifecycle</strong></h4><br />
<ol><br />
<li>The request first hits <code>public/index.php</code>.</li><br />
<li>CodeIgniter's routing (<code>app/Config/Routes.php</code>) matches the URL to a specific controller method.</li><br />
...
<ul><br />
<li><code>app/Commands</code>: Houses custom <code>spark</code> CLI commands, like the <code>train</code> command.</li><br />
<li><code>app/Config</code>: Contains all application configuration files, including <code>Routes.php</code> and <code>Services.php</code>.</li><br />
...
<li><code>app/Libraries</code>: This directory is used for the <strong>Service layer</strong>, containing all core business logic.</li><br />
...
<li><code>public/</code>: The web server's document root, containing the main <code>index.php</code> file and public assets.</li><br />
...
<li><strong>CSRF Protection:</strong> Cross-Site Request Forgery tokens are used on all POST forms to prevent malicious submissions.</li><br />
...
<li><strong>Environment Variables:</strong> Sensitive information like API keys and database credentials are stored in the <code>.env</code> file, which is never committed to version control.</li><br />
...
<ol><br />
<li><strong>Create the Controller</strong> using the command line: <code>php spark make:controller NoteController</code></li><br />
...
<pre><code class="language-php">&lt;?= $this-&gt;extend('layouts/default') ?&gt;<br />
...
<li><strong>6.1.2. Email Verification &amp; Password Resets</strong>: Upon registration, a unique token is generated and emailed to the user. <code>AuthController::verifyEmail()</code> handles this token. The password reset flow also uses a secure, expiring token sent via email.</li><br />
<li><strong>6.1.3. Access Control with Filters</strong>: The <code>AuthFilter</code> (<code>app/Filters/AuthFilter.php</code>) is applied to routes in <code>app/Config/Routes.php</code> to protect pages that require a user to be logged in.</li><br />
</ul><br />
...
<li><strong>6.2.2. Initiating a Transaction</strong>: <code>PaymentsController::initiate()</code> collects the amount and email, creates a local record in the <code>payments</code> table with a <code>pending</code> status, and calls <code>PaystackService::initializeTransaction()</code>. This service sends the request to Paystack, which returns a unique authorization URL. The user is then redirected to this URL to complete the payment.</li><br />
<li><strong>6.2.3. Verifying a Payment</strong>: After payment, Paystack redirects the user to the <code>callback_url</code> (<code>payment/verify</code>). <code>PaymentsController::verify()</code> retrieves the transaction reference and uses <code>PaystackService::verifyTransaction()</code> to confirm the payment status with Paystack. If successful, the local payment record is updated to <code>success</code> and the user's balance is updated within a database transaction.</li><br />
</ul><br />
<h4><strong>6.3. AI Service Integration</strong></h4><br />
<ul><br />
<li><strong>6.3.1. Generating Content</strong>: <code>GeminiController::generate()</code> is the core method. It prepares the user's prompt, adds contextual data from the memory system, and sends it to <code>GeminiService::generateContent()</code>. The service makes the API call to Google Gemini and returns the response.</li><br />
<li><strong>6.3.2. Conversational Memory System</strong>: This advanced feature is managed by <code>MemoryService.php</code>. When a user submits a prompt, the service:<br />
<ol><br />
<li>Generates a vector embedding of the user's input using <code>EmbeddingService</code>.</li><br />
<li>Performs a hybrid search (semantic vector search + keyword search) on past interactions stored in the database.</li><br />
...
<li>After receiving a response, it stores the new question-and-answer pair and updates the relevance scores of all memories.</li><br />
</ol></li><br />
<li><strong>6.3.3. Handling Multimedia Inputs</strong>: Users can upload files via the AI Studio. <code>GeminiController::uploadMedia()</code> handles the file, and <code>GeminiController::generate()</code> processes it, converting the file to base64 and including it in the API request to Gemini, which is a multimodal model.</li><br />
</ul><br />
<h4><strong>6.4. Cryptocurrency Data Service</strong></h4><br />
<ul><br />
<li><strong>6.4.1. Querying Balances</strong>: <code>CryptoController::query()</code> calls <code>CryptoService::getBtcBalance()</code> or <code>getLtcBalance()</code>. The service makes an API call to a third-party blockchain explorer (e.g., blockchain.info, blockchair.com) and formats the response.</li><br />
<li><strong>6.4.2. Fetching Transaction Histories</strong>: Similarly, the controller calls <code>CryptoService::getBtcTransactions()</code> or <code>getLtcTransactions()</code>. The service fetches the transaction data and formats it into a readable structure for the view.</li><br />
</ul><br />
<h4><strong>6.5. Administrative Dashboard</strong></h4><br />
<ul><br />
<li><strong>6.5.1. User Management</strong>: <code>AdminController.php</code> provides methods to list, search, view details, update balances, and delete users. All actions are protected to ensure only administrators can perform them. Balance updates are handled within a database transaction for data integrity.</li><br />
<li><strong>6.5.2. Sending Email Campaigns</strong>: <code>CampaignController.php</code> allows an administrator to compose an email that is then sent to every registered user in the <code>users</code> table. The process iterates through users and sends individual emails.</li><br />
</ul><br />
<hr /><br />
<h2><strong>Part III: Technical Reference</strong></h2><br />
<h3><strong>7. Command-Line Interface (CLI)</strong></h3><br />
<p>CodeIgniter's CLI tool, <code>spark</code>, is used for various development and maintenance tasks.</p><br />
<h4><strong>7.1. Overview of Custom Commands</strong></h4><br />
<p>You can list all available commands by running <code>php spark</code>. Custom application commands are located in <code>app/Commands/</code>.</p><br />
<h4><strong>7.2. <code>php spark train</code></strong></h4><br />
...
<li><strong>Action:</strong> It invokes <code>TrainingService::train()</code>, which reads a training dataset, processes it, trains a Naive Bayes classifier, and saves the serialized model files to the <code>writable/nlp/</code> directory. This ensures that performance-intensive model training does not impact the live web application.</li><br />
</ul><br />
<h4><strong>7.3. <code>php spark [another:command]</code></strong></h4><br />
<p>Other essential built-in commands include:</p><br />
<ul><br />
<li><code>php spark migrate</code>: Applies pending database migrations.</li><br />
<li><code>php spark db:seed &lt;SeederName&gt;</code>: Runs a database seeder to populate tables with data.</li><br />
<li><code>php spark make:controller &lt;Name&gt;</code>: Generates a new controller file.</li><br />
<li><code>php spark optimize</code>: Caches configuration and file locations for improved performance.</li><br />
</ul><br />
<h3><strong>8. Configuration Reference</strong></h3><br />
<h4><strong>8.1. Application (<code>App.php</code>)</strong></h4><br />
<p>Located at <code>app/Config/App.php</code>, this file contains the base configuration for the application, including the <code>baseURL</code>, <code>indexPage</code>, and <code>appTimezone</code>.</p><br />
<h4><strong>8.2. Database (<code>Database.php</code>)</strong></h4><br />
<p>Located at <code>app/Config/Database.php</code>, this file defines the connection parameters for your databases. The <code>default</code> group is used for the main application, while the <code>tests</code> group is used for PHPUnit testing.</p><br />
<h4><strong>8.3. Custom Configurations (<code>AGI.php</code>, etc.)</strong></h4><br />
<p>Custom configuration files are placed in <code>app/Config/Custom/</code>.</p><br />
<ul><br />
<li><code>AGI.php</code>: Contains all settings related to the AI service, including embedding models, hybrid search parameters, and memory logic (e.g., decay scores, context budget).</li><br />
<li><code>Recaptcha.php</code>: Stores the site and secret keys for the Google reCAPTCHA service, which are loaded from the <code>.env</code> file.</li><br />
</ul><br />
<h3><strong>9. Testing</strong></h3><br />
<h4><strong>9.1. Running the Test Suite</strong></h4><br />
<p>The project is configured to use PHPUnit for testing. The test suite can be run using a Composer script.</p><br />
<pre><code class="language-bash">composer test</code></pre><br />
<p>This command executes <code>phpunit</code> as defined in <code>composer.json</code> and uses the configuration from <code>phpunit.xml.dist</code>.</p><br />
<h4><strong>9.2. Writing Unit Tests</strong></h4><br />
<p>Unit tests focus on testing individual classes (like a Service or Model) in isolation. Test files should be placed in the <code>tests/</code> directory, mirroring the <code>app/</code> structure.</p><br />
<h4><strong>9.3. Writing Feature Tests</strong></h4><br />
<p>Feature tests are designed to test a full request-response cycle, simulating a user interacting with the application. They allow you to test controllers, views, and redirects together.</p><br />
<hr /><br />
<h2><strong>Part IV: Operations &amp; Community</strong></h2><br />
<h3><strong>10. Deployment</strong></h3><br />
<h4><strong>10.1. Production Server Setup</strong></h4><br />
<p>The <code>setup.sh</code> script provides a complete, automated setup for a production-ready Ubuntu server, including installing the web server, PHP, database, and all project dependencies, as well as configuring a virtual host.</p><br />
<h4><strong>10.2. Deployment Checklist</strong></h4><br />
<ol><br />
...
<h4><strong>10.3. Performance Optimization</strong></h4><br />
<ul><br />
<li><strong>Caching:</strong> The application can be configured to use various caching strategies. The <code>app/Config/Cache.php</code> file is set to use Redis as the primary handler with a file-based fallback.</li><br />
<li><strong>Autoloader Optimization:</strong> The <code>--optimize-autoloader</code> flag in the composer install command creates an optimized class map for faster class loading.</li><br />
<li><strong>Spark Optimize Command:</strong> <code>php spark optimize</code> caches configuration and speeds up the framework's file locator.</li><br />
<li><strong>Database Queries:</strong> Use pagination (<code>paginate()</code>) for lists and select only necessary columns to keep database interactions efficient.</li><br />
</ul><br />
<h3><strong>11. Troubleshooting</strong></h3><br />
<h4><strong>11.1. Frequently Asked Questions (FAQ)</strong></h4><br />
<ul><br />
<li><strong>Why did my payment fail?</strong> Ensure you have sufficient funds and that your payment provider (e.g., M-Pesa) is active. If the problem persists, contact support with your transaction reference.</li><br />
<li><strong>Why can't I log in after registering?</strong> You must click the verification link sent to your email address before you can log in.</li><br />
<li><strong>Why is my AI query failing?</strong> This could be due to insufficient balance or a temporary issue with the Gemini API. Check your balance and try again after a few moments.</li><br />
</ul><br />
<h4><strong>11.2. Common Error Resolutions</strong></h4><br />
<ul><br />
<li><strong>"Whoops! We hit a snag."</strong>: This is the generic production error message. Check the server logs at <code>writable/logs/</code> for the specific error details.</li><br />
<li><strong>"File upload failed."</strong>: This usually indicates a permissions issue. Ensure the <code>writable/uploads/</code> directory and its subdirectories are writable by the web server.</li><br />
<li><strong>"Could not send email."</strong>: Verify that your SMTP credentials in the <code>.env</code> file are correct and that your email provider is not blocking the connection.</li><br />
</ul><br />
<h4><strong>11.3. Logging &amp; Debugging</strong></h4><br />
<ul><br />
<li><strong>Log Location:</strong> All application logs are stored in <code>writable/logs/</code>, with a new file created for each day.</li><br />
<li><strong>Log Levels:</strong> The logging sensitivity can be adjusted in <code>app/Config/Logger.php</code>. In a <code>development</code> environment, the threshold is low to capture all messages. In <code>production</code>, it is higher to only log errors and critical issues.</li><br />
</ul><br />
<h3><strong>12. Contributing</strong></h3><br />
<h4><strong>12.1. Contribution Guidelines</strong></h4><br />
<ol><br />
...
<h4><strong>12.2. Code Style (PSR-12)</strong></h4><br />
<p>The project enforces the PSR-12 coding standard. All contributions must adhere to this standard.</p><br />
<h4><strong>12.3. Submitting a Pull Request</strong></h4><br />
<p>Before submitting a pull request, ensure that your code is well-documented, follows the project's architectural patterns (MVC-S), and that all existing tests pass.</p><br />
<h3><strong>13. Appendices</strong></h3><br />
<h4><strong>13.1. Glossary of Terms</strong></h4><br />
<ul><br />
...
<li><strong>Service:</strong> A class in the <code>app/Libraries</code> directory that contains reusable business logic.</li><br />
<li><strong>Entity:</strong> A class that represents a single row from a database table, allowing for object-oriented interaction with data.</li><br />
<li><strong>Embedding:</strong> A numerical vector representation of text, used for semantic understanding and similarity searches in the AI memory system.</li><br />
<li><strong>Pay-as-you-go:</strong> A pricing model where users pay only for the services they consume, rather than a fixed subscription fee.</li><br />
</ul><br />
<h4><strong>13.2. Changelog &amp; Release History</strong></h4><br />
<p><em>(This section would be maintained with a list of versions and the changes introduced in each release.)</em></p><br />
<ul><br />
...
<hr /><br />
<h3><strong>Part V: Documentation Maintenance Guide</strong></h3><br />
...
<pre><code class="language-bash">git diff main --name-status  | git diff main --name-status &gt; changed_files.txt<br />
git diff main  | git diff main &gt; code_changes.diff</code></pre><br />
</li><br />
<li><br />
...
<td style="text-align:left;"><strong>4. Core Concepts</strong> (if a fundamental service changed), <strong>6. Feature Guides</strong> (detailed business logic)</td><br />
</tr><br />
<tr><br />
...
<li><br />
<p><strong>Format the Entry:</strong> Use the following structure, inspired by <a href="https://keepachangelog.com/en/1.0.0/">Keep a Changelog</a>. Only include the categories you need for that release.</p><br />
