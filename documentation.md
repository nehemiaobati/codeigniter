...
- `OPENROUTER_API_KEY`: **(Required)** Your API key for OpenRouter models.
...
- **6.3.1. Generative Text & Resilience Strategy**:
  ...
  - **Thought Process Compatibility**: The system natively supports and renders "Thinking" blocks from advanced models (e.g., Gemini 2.0 Flash Thinking, DeepSeek-R1 via OpenRouter), providing transparency into the AI's reasoning. The frontend gracefully handles these blocks for both standard and streaming responses.
...
- **6.8.1. Configuration & Model Selection**: The module connects to a local Ollama instance (default: `http://localhost:11434`). It dynamically fetches available models (e.g., `llama3`, `mistral`) and presents them in the UI. 
  - **Performance Optimization**: Model listing is cached for 10 minutes to ensure instant page loads. If the cache is empty and the Ollama server is unreachable, the system fails fast with a 2-second timeout to prevent UI hang.
...
| `OPENROUTER_API_KEY`      | OpenRouter API Key.                 | OpenRouter Module |
...
### Added
- **OpenRouter & Ollama CSRF Bypass**: Exempted document download routes from CSRF protection for a smoother export experience.
- **OpenRouter "Thinking Process"**: Full formatting and rendering support for reasoning models.
- **Environment Template Updates**: Added `OPENROUTER_API_KEY` to `setup.sh` and `env` templates.

### Changed
- **Ollama Performance Optimization**: Implemented 10-minute caching for model list and reduced connection timeouts (2s) to prevent UI blocking when the local server is offline.
- **OpenRouter UI Consistency**: Resolved dual-rendered thinking blocks by centralizing formatting in the backend controller.
